...
#cov_stored_single_avg_y = data_y['cov_stored_single_avg']
...
Xrange = data_x['Xrange']/0.15#/np.sqrt(data_x['theta'][0])
...
for i in range(2):
    for j in range(2):
        ax = fig.add_subplot(2, 2, j+1+i*2)
        plt.grid()
        ...
        if i == 1: plt.xlabel(r' ', fontsize = 18, fontweight='bold')
        ...
        if i == 0:
            if j == 0:
                ax.set_title(r'$\Delta p_1$ step (in-plane)', fontsize = 16, fontweight='bold')
            elif j == 1:
                ax.set_title(r'$\Delta p_3$ step (out-of-plane)', fontsize = 16, fontweight='bold')
            #elif j == 2:
                #ax.set_title(r'step in $x_3$ direction')

# Share xlabel for every column
fig.text(0.5, 0.025, r'Step distance $ \mathbf{ \Delta p/ l}$ [$-$]', ha='center', fontsize = 16, fontweight='bold')

...
my_path = os.getcwd()
my_data = 'Data'
data_x = np.load(os.path.join(my_path, my_data, 'MC_step_x.npz'))
data_y = np.load(os.path.join(my_path, my_data, 'MC_step_y.npz'))
data_z = np.load(os.path.join(my_path, my_data, 'MC_step_z.npz'))
...
for i in range(2):
    ...
    # for j in range(2):
        ...
        # ax = fig.add_subplot(2, 1, j+1+i*2)
...
fig.text(0.5, 0.025, r'Sensor separation $\alpha_l$ [$-$]', ha='center', fontsize = 16, fontweight='bold')

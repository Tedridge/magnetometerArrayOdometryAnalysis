...
def processTimestep(timestep):
    ...
    return dx_stored_local, dx_est_stored_local, eta_error_stored_local, R_est_stored_local, R_true_stored_local, StoreLinAlgError_local, cov_stored_local
...
for magnetometers, magName in zip(magmagnetometersNumbers, magNames):
    for scenario in Scenarios:
        ...
        # m['mag_D'], m['mag_o'], m['SigmaY'] = process_calibrationdata(Data)

        # # my_file = 'bigsquare3.mat'
        ...
        num_jobs = -1  # Use # available CPU cores
        ...
        for initial_condition in Initial_conditions:
            ...
            for i, (dx_stored_local, dx_est_stored_local, eta_error_stored_local, R_est_stored_local, R_true_stored_local, StoreLinAlgError_local, cov_stored_local) in enumerate(results):
            ...
            np.savez(os.path.join(my_path, my_data, my_file),Xcenter=Xcenter, Xdata=Xdata, Rdata=Rdata, dx_stored=dx_stored, dx_est_stored=dx_est_stored, R_est_stored=R_est_stored, R_true_stored=R_true_stored, eta_error_stored=eta_error_stored, cov_stored=cov_stored, StoreLinAlgError=StoreLinAlgError, ydata_n=ydata_n)
